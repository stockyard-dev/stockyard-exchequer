...
async function load(){var r=await fetch(A+'/budgets').then(function(r){return r.json()});items=r.budgets||[];document.getElementById('stats').innerHTML='<div class="st"><div class="st-v">'+items.length+'</div><div class="st-l">Total</div></div><div class="st"><div class="st-v">-</div><div class="st-l">&nbsp;</div></div><div class="st"><div class="st-v">-</div><div class="st-l">&nbsp;</div></div>';render();}
function render(){var q=(document.getElementById('search').value||'').toLowerCase();var f=items;
if(q)f=f.filter(function(i){return (i.name||'').toLowerCase().includes(q)||(i.category||'').toLowerCase().includes(q)});
if(!f.length){document.getElementById('list').innerHTML='<div class="empty">No budgets.</div>';return;}
...
if(i.allocated)h+='<span>'+esc(String(i.allocated))+'</span>';
if(i.spent)h+='<span>'+esc(String(i.spent))+'</span>';
if(i.period)h+='<span>'+esc(String(i.period))+'</span>';
if(i.start_date)h+='<span>'+esc(String(i.start_date))+'</span>';
h+='<span>'+ft(i.created_at)+'</span></div>';
h+='</div>';});
document.getElementById('list').innerHTML=h;}
async function del(id){if(!confirm('Delete?'))return;await fetch(A+'/budgets/'+id,{method:'DELETE'});load();}
function formHTML(item){var i=item||{name:'',category:'',allocated:'',spent:'',period:'',start_date:'',end_date:'',notes:''};var isEdit=!!item;
var h='<h2>'+(isEdit?'EDIT':'NEW')+' BUDGET</h2>';
...
h+='<div class="fr"><label>Allocated</label><input id="f-allocated" value="'+esc(i.allocated||'')+'" ></div>';
h+='<div class="fr"><label>Spent</label><input id="f-spent" value="'+esc(i.spent||'')+'" ></div>';
h+='<div class="fr"><label>Period</label><input id="f-period" value="'+esc(i.period||'')+'" ></div>';
h+='<div class="fr"><label>Start Date</label><input id="f-start_date" value="'+esc(i.start_date||'')+'" ></div>';
h+='<div class="fr"><label>End Date</label><input id="f-end_date" value="'+esc(i.end_date||'')+'" ></div>';
...
async function submit(){var body={name:document.getElementById('f-name').value.trim(),category:document.getElementById('f-category').value.trim(),allocated:document.getElementById('f-allocated').value.trim(),spent:document.getElementById('f-spent').value.trim(),period:document.getElementById('f-period').value.trim(),start_date:document.getElementById('f-start_date').value.trim(),end_date:document.getElementById('f-end_date').value.trim(),notes:document.getElementById('f-notes').value.trim()};
if(editId){await fetch(A+'/budgets/'+editId,{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});}
else{await fetch(A+'/budgets',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});}closeModal();load();}
