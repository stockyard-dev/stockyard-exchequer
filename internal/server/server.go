...
import (
	...
	"github.com/stockyard-dev/stockyard-exchequer/internal/store"
)

const resourceName = "budgets"
...
func New(db *store.DB, limits Limits, dataDir string) *Server {
	...
	s.mux.HandleFunc("GET /api/tier", func(w http.ResponseWriter, r *http.Request) {
		wj(w, 200, map[string]any{
			"tier":        s.limits.Tier,
			"upgrade_url": "https://stockyard.dev/exchequer/",
		})
	})
...
func (s *Server) loadPersonalConfig() {
	...
	if err := json.Unmarshal(data, &cfg); err != nil {
		log.Printf("exchequer: warning: could not parse config.json: %v", err)
		return
	}
	s.pCfg = cfg
	log.Printf("exchequer: loaded personalization from %s", path)
}
...
func (s *Server) list(w http.ResponseWriter, r *http.Request) {
	...
	if q != "" || len(filters) > 0 {
		wj(w, 200, map[string]any{"budgets": oe(s.db.Search(q, filters))})
		return
	}
...
func (s *Server) create(w http.ResponseWriter, r *http.Request) {
	if s.limits.MaxItems > 0 && s.db.Count() >= s.limits.MaxItems {
		we(w, 402, "Free tier limit reached. Upgrade at https://stockyard.dev/exchequer/")
		return
	}
	var e store.Budget
...
// update accepts a partial budget. Uses json.RawMessage map for proper
// PATCH semantics. This is critical here because Allocated and Spent are
// the most important fields to update — and resetting Spent to 0 at the
// start of a new period is a totally valid operation that the old
// 'if x == 0 preserve' pattern would have silently broken.
func (s *Server) update(w http.ResponseWriter, r *http.Request) {
	...
	if v, ok := raw["allocated"]; ok {
		json.Unmarshal(v, &patch.Allocated)
	}
	if v, ok := raw["spent"]; ok {
		json.Unmarshal(v, &patch.Spent)
	}
	...
	if v, ok := raw["start_date"]; ok {
		json.Unmarshal(v, &patch.StartDate)
	}
	if v, ok := raw["end_date"]; ok {
		json.Unmarshal(v, &patch.EndDate)
	}
