package server
import("encoding/json";"net/http";"github.com/stockyard-dev/stockyard-exchequer/internal/store")
...
func(s *Server)routes(){
    s.mux.HandleFunc("GET /health",func(w http.ResponseWriter,r *http.Request){writeJSON(w,200,map[string]string{"status":"ok","service":"stockyard-exchequer"})})
    s.mux.HandleFunc("GET /api/stats",s.handleOverview)
    s.mux.HandleFunc("GET /api/budgets",s.handleList)
    s.mux.HandleFunc("POST /api/budgets",s.handleCreate)
    s.mux.HandleFunc("POST /api/budgets/{id}/actuals",s.handleAddActual)
    s.mux.HandleFunc("DELETE /api/budgets/{id}",s.handleDelete)
